...
class PostsListView(ListView):
    ...
    def get_queryset(self) -> QuerySet[Any]:
        ...
        queryset = queryset.filter(Q(is_published=True)).select_related('name_user')
        return queryset
...
class PostDetailView(ModelFormMixin, DetailView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        moderator = get_or_set_cache(self.request.user.groups, slug='moderator', type_field='name')
        context = super().get_context_data(**kwargs)
        context['title'] = 'Electronic Shop'
        context['catg_selected'] = 7
        context['comments'] = PostComment.objects.filter(Q(post=kwargs['object'])).select_related('parent', 'user_name', 'post')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if check_values.get('text'):
            ...
            comment = PostComment.objects.filter(pk=check_values.get('get_block_comment')).first()
            if not comment:
...
class UpdatePostView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self) -> bool | None:
        obj = self.get_object()
        return self.request.user == obj.name_user or self.request.user.is_superuser
