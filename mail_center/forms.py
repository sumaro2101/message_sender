...
class FormSendMesssage(ModelForm):
    ...
    date_first_send = fields.DateTimeField(widget=DateTimeInput(attrs={'class': 'form-control form-control-sm'}), required=False, label='Начало отправки')
    ...
    def clean_date_first_send(self):
        ...
        if date_first_send:
            if date_first_send < datetime.now(timezone.utc) - timedelta(seconds=30):
