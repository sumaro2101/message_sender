...
@app.task
def send(object_unique_name: Union[str, None]= None, template_render: Union[str, None]=None) -> None:
    ...
    sheduler_task = PeriodicTask._default_manager.get(name=object_unique_name)
    model_check = SendingMessage._default_manager.select_related('message').get(pk=pk)
    ...
    try:
        ...
        ResultsSendMessages.objects.create(send_task=model_check, sheduler_task=sheduler_task, result='success')
    except SMTPException as e:
        print(e)
        ResultsSendMessages.objects.create(send_task=model_check, sheduler_task=sheduler_task, result='failed')
